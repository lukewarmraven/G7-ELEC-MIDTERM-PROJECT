...
ggplot(data, aes(x = age)) +
  ...
  # showing accurate count for each bin
...
ggplot(data, aes(x = sysBP)) +
  ...
  # showing accurate count for each bin
  # geom_text(stat = "bin", aes(label = ..count..), vjust = -0.5, color = "black", binwidth = 50) +
  # show text for each bin 
  stat_bin(binwidth=50, geom='text', aes(label=..count..), position=position_stack(vjust=0.5), color="black",size = 4, fontface="bold",family = "roboto") +
  theme(
    plot.background = element_rect(fill = "#FFFFFA", color = "#FFFFFA"),
    panel.background = element_rect(fill = "#FAF2F0", color = "#FAF2F0")
...
ggplot(data, aes(x = diaBP)) +
  ...
  # showing accurate count for each bin
  #geom_text(stat = "bin", aes(label = ..count..), vjust = -0.5, color = "black", binwidth = 10) +
  # show text for each bin 
  stat_bin(binwidth=50, geom='text', aes(label=..count..), position=position_stack(vjust=0.5), color="black",size = 4, fontface="bold",family = "roboto") +
  ylim(0,3000) +
  theme(
...
# sysbp, diabp, bmi, heartrate ###################################################
# Summarize the data to calculate means of SysBP, DiaBP, and HeartRate for each bmi_group
mean_data <- data %>%
  filter(!is.na(BMI)) %>%
...
# DENSITY PLOT
# Calculate the means of SysBP, DiaBP, and HeartRate
mean_values <- data %>%
  summarise(
    mean_SysBP = mean(sysBP, na.rm = TRUE),
    mean_DiaBP = mean(diaBP, na.rm = TRUE),
